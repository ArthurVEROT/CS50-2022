...
void sort_pairs(void)
{
    ...
    for (int i = 0; i < pair_count; ++i)
    {
        for (int j = 0; j < pair_count - 1 - i; ++j)
        {
            ...
            {
                // Swap for winner
                temp_memory = pairs[j].winner;
                pairs[j].winner = pairs[j + 1].winner;
                pairs[j + 1].winner = temp_memory;
                // Swap for loser
                temp_memory = pairs[j].loser;
                pairs[j].loser = pairs[j + 1].loser;
                pairs[j + 1].loser = temp_memory;
            }
        }
    }
    return;
...
void lock_pairs(void)
{
    ...
    for (int i = 0; i < pair_count; i++)
    {
        // If cycle there is no cycle, lock the pair
...
// check for cyle in the locked array
bool cycle(int end, int start)
{
    ...
    // Return true if there is a cycle created (Loop through candidates) (Recursive case)
    for (int i = 0; i < candidate_count; i++)
    {
